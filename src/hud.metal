// hud.metal — the HUD's SDF pass (ADR-0014, #359). The fragment shader locked in
// prototypes/hud-micro-motion (track B), proven from pure Zig by spike #356: every mark is a
// rounded box, unioned with a polynomial smooth-min (the "goo") and wrapped in an
// exponential soft glow in its own colour. One full-screen triangle; the uniforms go in via
// setFragmentBytes, so there are no buffers. Compiled at runtime by MetalChrome.init.
#include <metal_stdlib>
using namespace metal;

#define MAXP 32

// Must match `Uniforms` in hud.zig byte for byte (1184 bytes).
struct U {
