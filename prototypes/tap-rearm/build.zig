...
pub fn build(b: *std.Build) void {
    ...
    m.linkFramework("CoreFoundation", .{}); // run loop, CFRelease, CFMachPortInvalidate
    m.linkFramework("AppKit", .{}); // NSApplication activation policy (Sequoia headless test)
    ...
    const run_step = b.step("run", "Build and run the tap re-arm spike");
