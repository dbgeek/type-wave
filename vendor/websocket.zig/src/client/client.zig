    ...
    pub fn read(self: *Stream, buf: []u8) !usize {
        if (self.tls_client) |tls_client| {
            ...
            while (true) {
                ...
                // type-wave §3.5 fix (see docs/research/zig-websocket-tls.md): also skip
                // the poll when encrypted bytes are already buffered in the TLS input
                // reader — otherwise Cloudflare's burst delivery starves handshake reads
                // and deterministically times out against api.openai.com.
                if (tls_client.client.reader.bufferedLen() == 0 and
                    tls_client.client.input.bufferedLen() == 0 and
